...
class Danbooru(BaseBooru):
    ...
    def prepare_tags(self, tags: str) -> Tuple[str, List[str]]:
        ...
        # 2. Prioritize: Meta tags (like order:score) > Regular tags > Special tags (~, -)
        meta_tags = [t for t in all_tags if ":" in t and not t.startswith(("~", "-"))]
        regular_tags = [t for t in all_tags if t not in meta_tags and not t.startswith(("~", "-"))]
        special_tags = [t for t in all_tags if t.startswith(("~", "-"))]
